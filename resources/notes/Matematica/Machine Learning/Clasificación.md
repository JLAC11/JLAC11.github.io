# Clasificación

Cae dentro del *aprendizaje supervisado*.
Entonces, sean las variables predictoras $X\in\mathcal{X}\subseteq \mathbb{R}^p$ y las variables a clasificar $Y\in\mathcal{Y}=\{1,...,M\}$
Deseamos el mejor clasificador que minimice alguna *función de pérdida*:
$$\mathcal{L}:\mathcal{X}\times\mathcal{Y}\rightarrow\mathbb{R}^+$$
Es decir, buscamos que minimice:
$$\mathcal{L}\left(Y,g(X)\right)$$
Para que esté bien definido el problema tomamos el valor esperado:
$$g^* \in \arg \min_{g:\mathcal{X\rightarrow Y}} \mathbb{E}_{Y|X}\left[\mathcal{L}(Y,g(X))\right]$$
En este caso, $g^*$ es llamado el clasificador óptimo de Bayes.

### ¿Qué función de pérdida usar?

Si ningún error de clasificación es peor que otro:
$$\mathcal{L}(Y,g(X)) = \mathbb{1}_{g(X)\ne Y}$$

Esta función de pérdida es llamada la función de pérdida 0-1.
$$\mathbb{E}_{Y|X}\left[\mathcal{L}(Y,g(X))\right] = \mathbb{E}_{Y|X}\left[\mathbb{1}_{g(X)\ne Y}\right]
$$
Debido a que esta función de pérdida meramente puede tomar valores 0 y 1, entonces es una distribución de Bernoulli, o puesto de otra forma:
$$\mathbb{E}_{Y|X}\left[\mathbb{1}_{g(X)\ne Y}\right] = \mathbb{P}\left(g(X)\ne Y|X \right)$$
Por lo que se tiene como función de pérdida:
$$g^* \in \arg \max_{g:\mathcal{X\rightarrow Y}} \mathbb{P}\left(g(X) = Y|X \right)$$

Sin embargo, $g^*$ no se puede obtener si se desconoce la distribución de $Y|X$. Por lo tanto, recurrimos a datos $\mathcal{D}_n=((X_1,Y_1),...,(X_n,Y_n))$
Donde $(X_i,Y_i)\stackrel{iid}{\sim}(X_n,Y_n)$

El objetivo será construir a partir de $\mathcal{D}_n$ un clasificador $g_n:\mathcal{X\rightarrow Y}$ tal que:
$$\begin{align}
g_n&\in \arg \min_{g:\mathcal{X\rightarrow Y}} \mathbb{E}_{F_n}\left[\mathcal{L}(Y,g(X))\right]\\
&= \frac{1}{n}\sum_{i=1}^n\mathcal{L}(Y_i,g(X_i))
\end{align}$$
Si se usa la función indicadora, entonces:
$$\frac{1}{n}\sum_{i=1}^n\mathbb{1}_{g(X_i)\ne Y_i}$$

Si no son IID entonces hay que recurrir a causalidad :(

Otra posible es la curva [ROC](https://en.wikipedia.org/wiki/Receiver_operating_characteristic), a la que se toma el área bajo su curva.
---

excalidraw-plugin: parsed

---

==⚠  Switch to EXCALIDRAW VIEW in the MORE OPTIONS menu of this document. ⚠==

# Text Elements

Receiver Operating Characteristic ^U4Cli66A

TPR ^qL7TWqSe

FPR ^fOOjE2Qj

%%

# Drawing

````json
{
	"type": "excalidraw",
	"version": 2,
	"source": "https://excalidraw.com",
	"elements": [
		{
			"id": "U4Cli66A",
			"type": "text",
			"x": -153.47296142578125,
			"y": -262.7468566894531,
			"width": 333,
			"height": 25,
			"angle": 0,
			"strokeColor": "#000000",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "solid",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "sharp",
			"seed": 1648739655,
			"version": 61,
			"versionNonce": 244656231,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646882911632,
			"text": "Receiver Operating Characteristic",
			"rawText": "Receiver Operating Characteristic",
			"fontSize": 20,
			"fontFamily": 1,
			"textAlign": "left",
			"verticalAlign": "top",
			"baseline": 18,
			"containerId": null,
			"originalText": "Receiver Operating Characteristic"
		},
		{
			"id": "IlaGAW-hsu5PNJcKRgscX",
			"type": "line",
			"x": -160.5957697856985,
			"y": -217.44238367998972,
			"width": 1.6171055805124297,
			"height": 203.40692225420847,
			"angle": 0,
			"strokeColor": "#000000",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "solid",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "round",
			"seed": 788346249,
			"version": 48,
			"versionNonce": 1778455849,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646882931157,
			"points": [
				[
					0,
					0
				],
				[
					-1.6171055805124297,
					203.40692225420847
				]
			],
			"lastCommittedPoint": null,
			"startBinding": null,
			"endBinding": null,
			"startArrowhead": null,
			"endArrowhead": null
		},
		{
			"id": "9OHhjqg49DLa9P36Uu1It",
			"type": "line",
			"x": -161.6024627685547,
			"y": -15.5615234375,
			"width": 272.70581975765526,
			"height": 0.6489006642065931,
			"angle": 0,
			"strokeColor": "#000000",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "solid",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "round",
			"seed": 266707401,
			"version": 100,
			"versionNonce": 89288073,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646882925977,
			"points": [
				[
					0,
					0
				],
				[
					272.70581975765526,
					0.6489006642065931
				]
			],
			"lastCommittedPoint": null,
			"startBinding": null,
			"endBinding": null,
			"startArrowhead": null,
			"endArrowhead": null
		},
		{
			"id": "qL7TWqSe",
			"type": "text",
			"x": -200.46372985839844,
			"y": -125.0467529296875,
			"width": 44,
			"height": 25,
			"angle": 4.6705549876182655,
			"strokeColor": "#000000",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "solid",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "sharp",
			"seed": 1463459561,
			"version": 106,
			"versionNonce": 2002046183,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646882953025,
			"text": "TPR",
			"rawText": "TPR",
			"fontSize": 20,
			"fontFamily": 1,
			"textAlign": "left",
			"verticalAlign": "top",
			"baseline": 18,
			"containerId": null,
			"originalText": "TPR"
		},
		{
			"id": "fOOjE2Qj",
			"type": "text",
			"x": -33.588897705078125,
			"y": -8.7666015625,
			"width": 39,
			"height": 25,
			"angle": 0,
			"strokeColor": "#000000",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "solid",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "sharp",
			"seed": 1348603881,
			"version": 49,
			"versionNonce": 202039241,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646882959230,
			"text": "FPR",
			"rawText": "FPR",
			"fontSize": 20,
			"fontFamily": 1,
			"textAlign": "left",
			"verticalAlign": "top",
			"baseline": 18,
			"containerId": null,
			"originalText": "FPR"
		},
		{
			"id": "sJP7m2pHi0wkv3R-c4fcE",
			"type": "freedraw",
			"x": -152.7538604736328,
			"y": -22.845977783203125,
			"width": 265.5294952392578,
			"height": 186.17587280273438,
			"angle": 0,
			"strokeColor": "#990000",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "solid",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "round",
			"seed": 912956743,
			"version": 238,
			"versionNonce": 1666518505,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646882977075,
			"points": [
				[
					0,
					0
				],
				[
					0,
					-0.305206298828125
				],
				[
					0,
					-0.61041259765625
				],
				[
					0,
					-0.915618896484375
				],
				[
					0,
					-1.2208251953125
				],
				[
					-0.305206298828125,
					-1.83123779296875
				],
				[
					-0.61041259765625,
					-3.05206298828125
				],
				[
					-0.61041259765625,
					-3.6624755859375
				],
				[
					-1.2208251953125,
					-4.578094482421875
				],
				[
					-1.2208251953125,
					-4.88330078125
				],
				[
					-1.2208251953125,
					-5.188507080078125
				],
				[
					-1.2208251953125,
					-5.49371337890625
				],
				[
					-1.2208251953125,
					-5.798919677734375
				],
				[
					-0.915618896484375,
					-6.409332275390625
				],
				[
					-0.915618896484375,
					-7.019744873046875
				],
				[
					-0.61041259765625,
					-7.93536376953125
				],
				[
					-0.61041259765625,
					-8.240570068359375
				],
				[
					-0.305206298828125,
					-9.461395263671875
				],
				[
					0,
					-10.37701416015625
				],
				[
					0.305206298828125,
					-10.682220458984375
				],
				[
					0.915618896484375,
					-11.903045654296875
				],
				[
					1.83123779296875,
					-14.03948974609375
				],
				[
					2.441650390625,
					-16.175933837890625
				],
				[
					3.357269287109375,
					-18.007171630859375
				],
				[
					3.967681884765625,
					-18.92279052734375
				],
				[
					4.578094482421875,
					-20.448822021484375
				],
				[
					5.188507080078125,
					-21.36444091796875
				],
				[
					5.798919677734375,
					-23.500885009765625
				],
				[
					7.019744873046875,
					-26.552947998046875
				],
				[
					7.93536376953125,
					-28.384185791015625
				],
				[
					7.93536376953125,
					-29.91021728515625
				],
				[
					8.240570068359375,
					-31.13104248046875
				],
				[
					8.5457763671875,
					-32.35186767578125
				],
				[
					9.15618896484375,
					-34.18310546875
				],
				[
					9.461395263671875,
					-35.709136962890625
				],
				[
					9.7666015625,
					-37.540374755859375
				],
				[
					10.071807861328125,
					-38.45599365234375
				],
				[
					11.292633056640625,
					-43.644500732421875
				],
				[
					12.208251953125,
					-46.696563720703125
				],
				[
					12.513458251953125,
					-48.22259521484375
				],
				[
					13.123870849609375,
					-50.359039306640625
				],
				[
					13.734283447265625,
					-53.411102294921875
				],
				[
					14.03948974609375,
					-56.157958984375
				],
				[
					14.344696044921875,
					-57.98919677734375
				],
				[
					14.64990234375,
					-59.21002197265625
				],
				[
					14.64990234375,
					-60.43084716796875
				],
				[
					14.955108642578125,
					-61.346466064453125
				],
				[
					15.26031494140625,
					-62.567291259765625
				],
				[
					15.26031494140625,
					-64.09332275390625
				],
				[
					15.8707275390625,
					-66.53497314453125
				],
				[
					16.48114013671875,
					-68.3662109375
				],
				[
					17.091552734375,
					-71.72348022460938
				],
				[
					17.396759033203125,
					-73.55471801757812
				],
				[
					18.007171630859375,
					-75.38595581054688
				],
				[
					18.3123779296875,
					-76.30157470703125
				],
				[
					18.617584228515625,
					-77.82760620117188
				],
				[
					19.533203125,
					-79.96405029296875
				],
				[
					19.533203125,
					-80.574462890625
				],
				[
					20.14361572265625,
					-81.7952880859375
				],
				[
					20.14361572265625,
					-82.40570068359375
				],
				[
					20.7540283203125,
					-83.62652587890625
				],
				[
					21.059234619140625,
					-84.54214477539062
				],
				[
					21.669647216796875,
					-85.457763671875
				],
				[
					22.280059814453125,
					-86.6785888671875
				],
				[
					22.890472412109375,
					-87.8994140625
				],
				[
					23.500885009765625,
					-88.50982666015625
				],
				[
					23.500885009765625,
					-88.81503295898438
				],
				[
					24.111297607421875,
					-89.42544555664062
				],
				[
					24.41650390625,
					-89.73065185546875
				],
				[
					24.721710205078125,
					-89.73065185546875
				],
				[
					25.02691650390625,
					-90.341064453125
				],
				[
					25.332122802734375,
					-90.341064453125
				],
				[
					25.942535400390625,
					-90.64627075195312
				],
				[
					26.24774169921875,
					-90.95147705078125
				],
				[
					26.552947998046875,
					-90.95147705078125
				],
				[
					26.858154296875,
					-91.25668334960938
				],
				[
					28.384185791015625,
					-91.86709594726562
				],
				[
					28.994598388671875,
					-92.47750854492188
				],
				[
					29.91021728515625,
					-92.78271484375
				],
				[
					30.5206298828125,
					-93.08792114257812
				],
				[
					31.13104248046875,
					-93.39312744140625
				],
				[
					31.741455078125,
					-93.69833374023438
				],
				[
					33.267486572265625,
					-94.30874633789062
				],
				[
					36.624755859375,
					-96.75039672851562
				],
				[
					37.23516845703125,
					-97.36080932617188
				],
				[
					37.8455810546875,
					-97.666015625
				],
				[
					38.761199951171875,
					-98.27642822265625
				],
				[
					39.67681884765625,
					-98.58163452148438
				],
				[
					46.391357421875,
					-107.12741088867188
				],
				[
					47.6121826171875,
					-108.95864868164062
				],
				[
					48.8330078125,
					-111.70550537109375
				],
				[
					50.66424560546875,
					-114.757568359375
				],
				[
					51.579864501953125,
					-116.28359985351562
				],
				[
					52.800689697265625,
					-119.03045654296875
				],
				[
					53.10589599609375,
					-120.25128173828125
				],
				[
					53.71630859375,
					-121.77731323242188
				],
				[
					54.631927490234375,
					-123.3033447265625
				],
				[
					55.852752685546875,
					-125.7449951171875
				],
				[
					56.463165283203125,
					-126.9658203125
				],
				[
					56.76837158203125,
					-128.1866455078125
				],
				[
					57.98919677734375,
					-130.32308959960938
				],
				[
					58.904815673828125,
					-131.23870849609375
				],
				[
					60.125640869140625,
					-134.290771484375
				],
				[
					60.43084716796875,
					-135.20639038085938
				],
				[
					60.736053466796875,
					-136.12200927734375
				],
				[
					61.346466064453125,
					-137.34283447265625
				],
				[
					61.65167236328125,
					-137.9532470703125
				],
				[
					61.956878662109375,
					-138.56365966796875
				],
				[
					62.2620849609375,
					-138.86886596679688
				],
				[
					62.2620849609375,
					-139.47927856445312
				],
				[
					62.567291259765625,
					-139.78448486328125
				],
				[
					62.87249755859375,
					-140.70010375976562
				],
				[
					63.177703857421875,
					-141.00531005859375
				],
				[
					63.177703857421875,
					-141.61572265625
				],
				[
					63.177703857421875,
					-141.92092895507812
				],
				[
					63.48291015625,
					-141.92092895507812
				],
				[
					63.48291015625,
					-142.22613525390625
				],
				[
					63.788116455078125,
					-142.53134155273438
				],
				[
					64.09332275390625,
					-143.14175415039062
				],
				[
					64.39852905273438,
					-143.44696044921875
				],
				[
					64.39852905273438,
					-143.75216674804688
				],
				[
					64.39852905273438,
					-144.057373046875
				],
				[
					64.7037353515625,
					-144.36257934570312
				],
				[
					65.00894165039062,
					-144.36257934570312
				],
				[
					65.00894165039062,
					-144.66778564453125
				],
				[
					65.61935424804688,
					-145.2781982421875
				],
				[
					65.924560546875,
					-145.58340454101562
				],
				[
					66.53497314453125,
					-145.88861083984375
				],
				[
					66.84017944335938,
					-146.19381713867188
				],
				[
					67.1453857421875,
					-146.80422973632812
				],
				[
					67.45059204101562,
					-147.10943603515625
				],
				[
					67.75579833984375,
					-147.7198486328125
				],
				[
					68.3662109375,
					-148.02505493164062
				],
				[
					68.67141723632812,
					-148.02505493164062
				],
				[
					69.89225769042969,
					-148.940673828125
				],
				[
					70.19746398925781,
					-149.24588012695312
				],
				[
					70.80787658691406,
					-149.55108642578125
				],
				[
					71.11308288574219,
					-149.85629272460938
				],
				[
					71.72349548339844,
					-150.16152954101562
				],
				[
					72.33390808105469,
					-150.46670532226562
				],
				[
					72.94432067871094,
					-150.77194213867188
				],
				[
					73.55473327636719,
					-151.38235473632812
				],
				[
					74.16514587402344,
					-151.38235473632812
				],
				[
					74.47035217285156,
					-151.68753051757812
				],
				[
					74.77555847167969,
					-151.68753051757812
				],
				[
					75.08076477050781,
					-151.68753051757812
				],
				[
					75.38597106933594,
					-151.68753051757812
				],
				[
					77.21720886230469,
					-152.29794311523438
				],
				[
					78.13282775878906,
					-152.60317993164062
				],
				[
					80.26927185058594,
					-153.21359252929688
				],
				[
					83.62654113769531,
					-154.12918090820312
				],
				[
					86.37339782714844,
					-155.04483032226562
				],
				[
					87.28901672363281,
					-155.65524291992188
				],
				[
					87.89942932128906,
					-155.96041870117188
				],
				[
					89.12025451660156,
					-156.57083129882812
				],
				[
					90.03587341308594,
					-157.18124389648438
				],
				[
					91.86711120605469,
					-158.09689331054688
				],
				[
					92.78273010253906,
					-158.40206909179688
				],
				[
					95.83479309082031,
					-159.92813110351562
				],
				[
					97.36082458496094,
					-160.53854370117188
				],
				[
					101.63371276855469,
					-162.67495727539062
				],
				[
					105.60139465332031,
					-164.50619506835938
				],
				[
					108.95867919921875,
					-166.33746337890625
				],
				[
					113.2315673828125,
					-168.168701171875
				],
				[
					119.03048706054688,
					-170.6103515625
				],
				[
					122.38775634765625,
					-171.22076416015625
				],
				[
					124.218994140625,
					-171.52597045898438
				],
				[
					126.96585083007812,
					-172.13638305664062
				],
				[
					132.15435791015625,
					-173.052001953125
				],
				[
					133.06997680664062,
					-173.052001953125
				],
				[
					139.78451538085938,
					-173.35720825195312
				],
				[
					142.2261505126953,
					-173.96762084960938
				],
				[
					147.1094512939453,
					-174.57803344726562
				],
				[
					149.5511016845703,
					-174.88323974609375
				],
				[
					153.51881408691406,
					-175.79885864257812
				],
				[
					155.3500518798828,
					-175.79885864257812
				],
				[
					159.0125274658203,
					-176.7144775390625
				],
				[
					162.36976623535156,
					-177.32489013671875
				],
				[
					163.28541564941406,
					-177.63009643554688
				],
				[
					165.1166534423828,
					-177.935302734375
				],
				[
					169.0843048095703,
					-178.85092163085938
				],
				[
					171.83119201660156,
					-179.46133422851562
				],
				[
					176.71449279785156,
					-180.376953125
				],
				[
					179.15614318847656,
					-180.68215942382812
				],
				[
					181.90296936035156,
					-180.98736572265625
				],
				[
					184.03944396972656,
					-181.5977783203125
				],
				[
					186.78627014160156,
					-181.90298461914062
				],
				[
					188.3123321533203,
					-181.90298461914062
				],
				[
					189.5331573486328,
					-182.20819091796875
				],
				[
					191.36439514160156,
					-182.20819091796875
				],
				[
					192.89039611816406,
					-182.20819091796875
				],
				[
					193.80604553222656,
					-182.20819091796875
				],
				[
					195.6372833251953,
					-182.51339721679688
				],
				[
					196.24769592285156,
					-182.51339721679688
				],
				[
					197.46852111816406,
					-182.51339721679688
				],
				[
					198.99452209472656,
					-182.51339721679688
				],
				[
					203.2674102783203,
					-182.818603515625
				],
				[
					205.09864807128906,
					-182.818603515625
				],
				[
					206.31947326660156,
					-182.818603515625
				],
				[
					207.23512268066406,
					-182.818603515625
				],
				[
					207.8455352783203,
					-182.818603515625
				],
				[
					211.20277404785156,
					-183.42901611328125
				],
				[
					212.42359924316406,
					-183.42901611328125
				],
				[
					214.86524963378906,
					-183.73422241210938
				],
				[
					217.30690002441406,
					-183.73422241210938
				],
				[
					217.9173126220703,
					-183.73422241210938
				],
				[
					220.96937561035156,
					-184.0394287109375
				],
				[
					222.19020080566406,
					-184.0394287109375
				],
				[
					224.32667541503906,
					-184.0394287109375
				],
				[
					230.1255645751953,
					-184.34463500976562
				],
				[
					234.39845275878906,
					-184.34463500976562
				],
				[
					238.06092834472656,
					-184.64984130859375
				],
				[
					242.0286407470703,
					-184.95504760742188
				],
				[
					245.6911163330078,
					-184.95504760742188
				],
				[
					247.52235412597656,
					-184.95504760742188
				],
				[
					249.04835510253906,
					-185.26025390625
				],
				[
					250.5744171142578,
					-185.26025390625
				],
				[
					251.7952423095703,
					-185.26025390625
				],
				[
					252.71083068847656,
					-185.56546020507812
				],
				[
					253.62648010253906,
					-185.56546020507812
				],
				[
					254.5420684814453,
					-185.56546020507812
				],
				[
					256.06813049316406,
					-185.56546020507812
				],
				[
					256.6785430908203,
					-185.56546020507812
				],
				[
					258.2045440673828,
					-185.56546020507812
				],
				[
					258.81495666503906,
					-185.56546020507812
				],
				[
					259.73060607910156,
					-185.56546020507812
				],
				[
					260.3410186767578,
					-185.56546020507812
				],
				[
					261.25660705566406,
					-185.56546020507812
				],
				[
					261.8670196533203,
					-185.56546020507812
				],
				[
					262.47743225097656,
					-185.87066650390625
				],
				[
					262.7826690673828,
					-185.87066650390625
				],
				[
					263.0878448486328,
					-185.87066650390625
				],
				[
					263.39308166503906,
					-185.87066650390625
				],
				[
					263.69825744628906,
					-185.87066650390625
				],
				[
					264.0034942626953,
					-186.17587280273438
				],
				[
					264.3086700439453,
					-186.17587280273438
				],
				[
					264.3086700439453,
					-186.17587280273438
				]
			],
			"pressures": [],
			"simulatePressure": true,
			"lastCommittedPoint": [
				264.3086700439453,
				-186.17587280273438
			]
		},
		{
			"id": "HxD1SY6oxtU-BQmAmqGBv",
			"type": "line",
			"x": -148.11851501464844,
			"y": -25.488037109375,
			"width": 254.43223571777344,
			"height": 180.76979064941406,
			"angle": 0,
			"strokeColor": "#446600",
			"backgroundColor": "transparent",
			"fillStyle": "hachure",
			"strokeWidth": 1,
			"strokeStyle": "dashed",
			"roughness": 1,
			"opacity": 100,
			"groupIds": [],
			"strokeSharpness": "round",
			"seed": 1924784423,
			"version": 28,
			"versionNonce": 1626952233,
			"isDeleted": false,
			"boundElements": null,
			"updated": 1646883015248,
			"points": [
				[
					0,
					0
				],
				[
					254.43223571777344,
					-180.76979064941406
				]
			],
			"lastCommittedPoint": null,
			"startBinding": null,
			"endBinding": null,
			"startArrowhead": null,
			"endArrowhead": null
		}
	],
	"appState": {
		"theme": "light",
		"viewBackgroundColor": "#ffffff",
		"currentItemStrokeColor": "#446600",
		"currentItemBackgroundColor": "transparent",
		"currentItemFillStyle": "hachure",
		"currentItemStrokeWidth": 1,
		"currentItemStrokeStyle": "dashed",
		"currentItemRoughness": 1,
		"currentItemOpacity": 100,
		"currentItemFontFamily": 1,
		"currentItemFontSize": 20,
		"currentItemTextAlign": "left",
		"currentItemStrokeSharpness": "sharp",
		"currentItemStartArrowhead": null,
		"currentItemEndArrowhead": "arrow",
		"currentItemLinearStrokeSharpness": "round",
		"gridSize": null
	},
	"files": {}
}
````

%%

El mejor clasificador es el [Clasificador óptimo de Bayes](Clasificador%20%C3%B3ptimo%20de%20Bayes.md).

|Regresión|Clasificación|
|---------|-------------|
|$Y\in\mathbb{R}$|$Y\in\mathcal{Y}=\{1,...,M\}$|
|$\mathbb{P}(Y=y  | X) = 0$|$\mathbb{P}(Y=y | X) \ge 0$|

### Teorema 2.1 (Devroye, Györfi,Lugosi)
